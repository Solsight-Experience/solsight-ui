...
const StatBadge: React.FC<{ value: number | string; variant?: 'default' | 'muted' }> = ({
  value,
  variant = 'default'
}) => (
  <span className={`text-[10px] px-1 py-0.5 rounded ${
    variant === 'muted' ? 'bg-gray-700 text-gray-400' : 'bg-gray-700 text-gray-300'
  }`}>
    {value}
  </span>
);
    ...
    <tr className="border-b border-gray-800/50 hover:bg-gray-800/30 text-[13px] group">
      ...
      <td className="py-2.5 px-2 whitespace-nowrap">
        <span className={`font-medium ${unrealized_pnl >= 0 ? 'text-green-400' : 'text-red-400'}`}>
          {unrealized_pnl >= 0 ? '+' : ''}{currencyFormatter.format(unrealized_pnl)}
        </span>
      </td>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="w-8 h-1 bg-gray-700 rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(balance_percent * 10, 100)}%` }}
            />
          </div>
        </div>
      </td>
        ...
        {funding_label ? (
          <div className="flex flex-col">
            ...
            <span className="text-gray-500 text-[10px]">
              {formatHeldTime(first_tx_time)} • ≡ 0.01 • 1
            </span>
          </div>
        ) : (
      ...
      <td className="py-2.5 px-2 whitespace-nowrap">
        <span className={`font-medium ${
          first_tx_time > Date.now() - 3600000 ? 'text-green-400' : 'text-gray-300'
        }`}>
