  ...
  return (
    <div
      className={`
        ...
        ${
          isUpdating
            ? 'border-purple-500 bg-black shadow-lg shadow-purple-500/20'
            : 'border-purple-600 bg-black hover:border-purple-500'
        }
        hover:shadow-md hover:shadow-purple-500/10
      `}
    >
      <span className="text-xs font-medium text-purple-400 uppercase tracking-wide mb-2">
        {label}
      </span>
      <span
        className={`text-lg sm:text-xl font-bold text-purple-300 transition-all duration-300 ${
...
export const TokenStats: React.FC<TokenStatsProps> = ({ token }) => {
        ...
        <div className="flex items-center justify-center h-40 border border-purple-100 rounded-xl bg-black">
          <div className="text-center">
            ...
            <p className="text-purple-500 text-xs mt-1">
              Select a token to view statistics
            </p>
          </div>
        </div>
      </div>
  ...
  const stats = [
    ...
    {
      label: '24h Tx',
      value: token.txns?.['24h']?.total
        ? `${(token.txns['24h'].total / 1000).toFixed(2)}K`
        : '-',
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
        {stats.map((stat) => (
          <StatCard
            ...
            value={stat.value}
            change={stat.change}
            isUpdating={updatingLabels.has(stat.label)}
          />
        ))}
      </div>
    </div>
