...
export const tokenApi = {
  ...
  getTopTraders: async (
    ...
    const response = await apiClient.get<TopTradersResponse>(
      TOKEN_ENDPOINTS.TOKEN_TOP_TRADERS(address),
      { params }
    );
    return response;
  },
  ...
  // Swap preview
  getSwapPreview: async (
    address: string,
    data: SwapPreviewRequest
  ): Promise<SwapPreviewResponse> => {
    const response = await apiClient.post<SwapPreviewResponse>(
      TOKEN_ENDPOINTS.SWAP_PREVIEW(address),
      data
    );
    return response;
  },
  ...
  addFavorite: async (address: string): Promise<{ success: boolean }> => {
    const response = await apiClient.post<{ success: boolean }>(
      TOKEN_ENDPOINTS.ADD_FAVORITE(address),
      {
  ...
  removeFavorite: async (address: string): Promise<{ success: boolean }> => {
    const response = await apiClient.delete<{ success: boolean }>(
      TOKEN_ENDPOINTS.REMOVE_FAVORITE(address)
    );
    return response;
  },
};
