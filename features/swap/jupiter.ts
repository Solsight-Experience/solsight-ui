...
export async function executeJupiterSwap(request: ExecuteSwapRequest, opts?: { config?: SwapApiConfig }): Promise<ExecuteSwapResult> {
    const config = opts?.config ?? getSwapApiConfig();
    ...
    if (!signature) {
        throw await withSwapErrorContext(sendError ?? new Error("Swap failed"), config.swapBaseUrl, config.quoteBaseUrl, lastTriedConnection);
    }
...
function getRpcCandidates(preferredRpcUrl: string): string[] {
    const configuredFallbacks =
        process.env.NEXT_PUBLIC_SOLANA_RPC_FALLBACK_URLS?.split(",")
            .map((url) => url.trim())
            .filter(Boolean) ?? [];
...
async function withSwapErrorContext(error: unknown, swapUrl: string, quoteUrl: string, connection: Connection | null): Promise<never> {
