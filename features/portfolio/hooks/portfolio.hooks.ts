...
export const portfolioKeys = {
    ...
    overview: (params?: Record<string, unknown>) => [...portfolioKeys.all, "overview", params] as const,
    pnlChart: (params: Record<string, unknown>) => [...portfolioKeys.all, "pnl-chart", params] as const,
    positions: (walletAddress: string, params?: Record<string, unknown>) => [...portfolioKeys.all, "positions", walletAddress, params] as const,
    activities: (params: Record<string, unknown>) => [...portfolioKeys.all, "activities", params] as const
};
...
export function useAddWallet() {
    ...
    return useMutation({
        mutationFn: (data: { address: string; name?: string; icon: string }) => portfolioApi.addWallet(data),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: portfolioKeys.all });
        }
    });
...
export function usePortfolioOverview(params?: { wallet_addresses?: string[]; time_frame?: "24h" | "7d" | "30d" | "90d" | "1y" }) {
    return useQuery({
...
export function usePnlChart(params: { wallet_addresses?: string[]; time_frame?: "7d" | "30d" | "90d" | "1y" | "all"; interval?: "1h" | "1d" | "1w" }) {
    return useQuery({
