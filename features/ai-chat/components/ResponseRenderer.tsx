...
const SEVERITY_CONFIG = {
    ...
    warning: {
        label: "Price Impact",
        className: "border-amber-500/20 bg-amber-500/5 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400/90"
    },
    danger: {
        label: "High Price Impact",
        className: "border-red-500/20 bg-red-500/5 dark:bg-red-500/10 text-red-600 dark:text-red-400/90"
    },
    critical: {
        label: "Very High Price Impact",
        className: "border-red-600/30 bg-red-600/10 dark:bg-red-600/20 text-red-600 dark:text-red-400/90 animate-pulse"
    }
} as const;
...
const TradeAutoAction: React.FC<{ data: TradeIntentData }> = ({ data }) => {
    ...
    return (
        <div className="relative overflow-hidden group p-4 rounded-2xl border border-violet-500/20 dark:border-violet-500/20 bg-[var(--surface-card)] dark:bg-gradient-to-br dark:from-violet-500/10 dark:via-transparent dark:to-indigo-500/10 shadow-lg border-[var(--border-subtle)]">
            {/* Background Glow Effect */}
            <div className="absolute -right-4 -top-4 w-24 h-24 bg-violet-500/5 dark:bg-violet-500/10 blur-2xl rounded-full group-hover:bg-violet-500/10 dark:group-hover:bg-violet-500/20 transition-all duration-500" />
            ...
            <div className="relative flex flex-col gap-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h4 className="text-sm font-semibold text-[var(--text-primary)]">Trade Preparation</h4>
                            <p className="text-[11px] text-[var(--text-muted)]">Redirecting to trading panel</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-2 py-1 rounded-lg bg-violet-500/10 border border-violet-500/20">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider text-violet-750 dark:text-white/70">{data.mode || "buy"}</span>
                    </div>
                </div>

                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4 px-4 py-3 rounded-xl bg-[var(--surface-panel)] border border-[var(--border-subtle)]">
                    <div className="flex flex-col gap-0.5">
                        <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-medium">Input</span>
                        <div className="flex items-center gap-2">
                            <Coins className="w-3.5 h-3.5 text-violet-600 dark:text-violet-400/80" />
                            <span className="text-xs font-semibold text-[var(--text-primary)] flex items-baseline gap-1">
                                {data.amount && <span className="text-violet-600 dark:text-violet-400 font-mono">{formatCompactNumber(data.amount)}</span>}
                    ...
                    <div className="flex flex-col gap-0.5 text-right">
                        <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-medium">Output</span>
                        <div className="flex items-center justify-end gap-2">
                            ...
                            <Coins className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400/80" />
                        </div>
                    </div>
                </div>
                ...
                {data.slippageBps !== undefined && (
                    <div className="flex flex-col gap-2">
                        <div className="grid grid-cols-2 gap-3 px-4 py-3 rounded-xl bg-[var(--surface-panel)] border border-[var(--border-subtle)]">
                            <div className="flex flex-col gap-0.5">
                                <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-medium">Slippage</span>
                                <span
                                    className={cn(
                                        "text-sm font-mono font-bold",
                                        data.slippageBps > 100 ? "text-amber-600 dark:text-amber-400" : "text-violet-600 dark:text-violet-400"
                                    )}
                                >
                                    {data.slippageBps} <span className="text-[10px] font-semibold text-[var(--text-muted)]">bps</span>
                                </span>
                            </div>
                            <div className="flex flex-col items-end gap-0.5">
                                <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider font-medium">Equivalent</span>
                                <span className="text-sm font-semibold text-[var(--text-primary)]">{(data.slippageBps / 100).toFixed(1)}%</span>
                            </div>
                        </div>

                        {data.slippageBps > 100 && (
                            <div className="px-3 py-2 rounded-xl border border-amber-500/20 bg-amber-500/5 dark:bg-amber-500/10 flex items-start gap-2 text-amber-700 dark:text-amber-400/90">
                                <AlertTriangle className="w-3.5 h-3.5 text-amber-650 dark:text-amber-500 shrink-0 mt-0.5" />
