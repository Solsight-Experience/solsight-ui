...
export const ChatBubble: React.FC<{ message: ChatMessageDto }> = ({ message }) => {
    ...
    return (
        <div className={cn("flex w-full gap-3 group animate-in fade-in slide-in-from-bottom-2 duration-300", isUser ? "flex-row-reverse" : "flex-row")}>
            ...
            <div className={cn("flex flex-col gap-1 max-w-[85%] sm:max-w-[75%]", isUser ? "items-end" : "items-start")}>
                ...
                >
                    <span className="text-xs font-medium text-foreground/80">{isUser ? "You" : "Assistant"}</span>
                ...
                {isUser ? (
                    ...
                    <div className="bg-card text-card-foreground border border-border px-4 py-2.5 rounded-2xl rounded-tl-sm text-sm leading-relaxed shadow-sm w-full">
                        {message.content && <div className="whitespace-pre-wrap mb-2 last:mb-0 break-words">{message.content}</div>}
