 ...
 */
export const SortPanel = memo<SortPanelProps>(function SortPanel({
    sortState,
    onSortChange,
}) {
    const renderSortButton = (option: SortOption, label: string) => {
        ...
        return (
            <Button
                ...
                className={cn(
                    'flex items-center gap-1 px-3 py-2 font-medium',
                    isActive && direction !== 'none' && 'text-brand-200'
                )}
            >
                <span>{label}</span>
                <div className="relative w-4 h-6">
                    <ChevronUp
                        ...
                        className={cn(
                            ...
                            isActive && direction === 'asc' ? 'text-brand-200' : 'text-neutral-500'
                        )}
                    />
                    <ChevronDown
                        size={16}
                        className={cn(
                            ...
                            isActive && direction === 'desc' ? 'text-brand-200' : 'text-neutral-500'
                        )}
                    />
                </div>
            </Button>
