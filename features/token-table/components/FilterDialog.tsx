...
export interface FilterFormData {
    // Metrics
    age_min_minutes: string;
    age_max_minutes: string;
...
export function getFilterRequestBody(
    formData: FilterFormData,
    filterType: 'token' | 'pool'
): TokenFilterRequest | PoolFilterRequest {
    if (filterType === 'token') {
        ...
        <FilterListContainer className="grid grid-cols-1 md:grid-cols-2 px-4">
            <FilterCheckBoxField
                ...
                onCheckedChange={(checked) =>
                    handleCheckboxChange('mint_authority_disabled', checked)
                }
            />
            <FilterCheckBoxField
                ...
                onCheckedChange={(checked) =>
                    handleCheckboxChange('freeze_authority_disabled', checked)
                }
            />
            <FilterCheckBoxField
            ...
            <FilterCheckBoxField
                ...
                onCheckedChange={(checked) => handleCheckboxChange('lp_burnt', checked)}
            />
        </FilterListContainer>
    );
}
    ...
    const handleCategoryToggle = (slug: string, checked: boolean) => {
        if (slug === 'all') {
            ...
            const newCategories = checked
                ? [...formData.categories, slug]
                : formData.categories.filter((c) => c !== slug);
    ...
    return (
        <FilterListContainer className="grid grid-cols-1 px-4">
            <FilterCheckBoxField
                ...
                onCheckedChange={(checked) => handleCategoryToggle('all', checked)}
            />
