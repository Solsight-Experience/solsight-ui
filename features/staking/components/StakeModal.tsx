...
import { MINIMUM_STAKE_SOL, MIN_RESERVE_SOL, STAKE_ACCOUNT_RENT_SOL } from '../constants/validators';
...
export function StakeModal({ open, onClose, walletPubkey, solBalance }: StakeModalProps) {
  const [amount, setAmount] = useState('');
  const { stakeState, handleStake } = useStaking();

  const loading = stakeState.status !== 'idle' && stakeState.status !== 'done' && stakeState.status !== 'error';
  const maxStakeable = Math.max(0, solBalance - MIN_RESERVE_SOL - STAKE_ACCOUNT_RENT_SOL);

  const handleSubmit = async () => {
    const parsed = parseFloat(amount);
    if (isNaN(parsed) || parsed <= 0) return;
    await handleStake(parsed, walletPubkey ?? '');
    if (stakeState.status === 'done') setAmount('');
  };

  const amountNum = parseFloat(amount);
  const isValid = !isNaN(amountNum) && amountNum >= MINIMUM_STAKE_SOL && amountNum <= maxStakeable && !!walletPubkey;

  return (
    <Dialog open={open} onOpenChange={() => { if (!loading) onClose(); }}>
      <DialogContent
        className="sm:max-w-md border-0 text-white p-0 overflow-hidden"
        style={{ background: 'linear-gradient(145deg, #110820 0%, #080612 100%)', boxShadow: '0 25px 60px rgba(139,92,246,0.25)' }}
      >
        {/* Header gradient bar */}
        <div className="h-1 w-full" style={{ background: 'linear-gradient(90deg, #8b5cf6, #3b82f6, #8b5cf6)', backgroundSize: '200% 100%', animation: 'shimmer-border 3s linear infinite' }} />
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Validator card */}
          <div className="flex items-center gap-3 rounded-2xl border border-green-500/25 bg-green-500/8 px-4 py-3.5">
            <ShieldCheck className="h-9 w-9 text-green-400 flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="font-bold text-white text-[13px]">Helius Validator</p>
              <p className="text-[11px] text-gray-400 mt-0.5">0% commission · High performance · MEV rewards</p>
            </div>
            <div className="flex items-center gap-1 rounded-full border border-green-500/30 bg-green-500/10 px-2.5 py-1">
              <Zap className="h-3 w-3 text-green-400" />
              <span className="text-green-400 font-bold text-[12px]">~7% APY</span>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/8 bg-white/4 px-4 py-3.5">
            ...
            <p className="text-3xl font-extrabold text-white">{solBalance.toFixed(4)} <span className="text-lg text-gray-400 font-semibold">SOL</span></p>
          </div>
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-600 mt-2">
              Min {MINIMUM_STAKE_SOL} SOL · Reserve {(MIN_RESERVE_SOL + STAKE_ACCOUNT_RENT_SOL).toFixed(5)} SOL for gas + rent
            </p>
          </div>
          ...
          <div className="flex gap-2.5 rounded-2xl border border-blue-500/20 bg-blue-500/6 px-3.5 py-3 text-[12px] text-blue-300">
            ...
            <span className="leading-relaxed">
              SOL stays in your wallet as a Stake Account. Becomes active at the next epoch (~2 days).
              You keep <strong className="text-white">100%</strong> of rewards.
            </span>
          </div>
          ...
          {stakeState.status === 'done' && stakeState.signature && (
            <p className="text-[12px] text-green-400 rounded-xl bg-green-400/8 px-3.5 py-2.5 border border-green-400/20">
              Staked successfully!{' '}
              <a href={`https://solscan.io/tx/${stakeState.signature}`} target="_blank" rel="noopener noreferrer" className="underline font-semibold">
                View on Solscan ↗
              </a>
            </p>
          )}
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              ...
              style={{ background: isValid && !loading ? 'linear-gradient(135deg, #8b5cf6 0%, #3b82f6 100%)' : 'rgba(255,255,255,0.05)', boxShadow: isValid && !loading ? '0 4px 20px rgba(139,92,246,0.30)' : 'none' }}
