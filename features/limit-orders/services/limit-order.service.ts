...
export class LimitOrderService {
  ...
  static async createOrder(request: CreateLimitOrderRequest): Promise<CreateLimitOrderResponse> {
    const response = await apiClient.post<{ success: boolean; data: CreateLimitOrderResponse }>(
  ...
  static async executeOrder(request: ExecuteLimitOrderRequest): Promise<ExecuteLimitOrderResponse> {
    const response = await apiClient.post<{ success: boolean; data: ExecuteLimitOrderResponse }>(
      '/api/limit-orders/execute',
      request
    );
    if (!response.success || !response.data) {
  ...
  static async getOrders(
    ...
    const response = await apiClient.get<{ success: boolean; data: GetOrdersResponse }>(
      `/api/limit-orders?${params.toString()}`
    );
    if (!response.success || !response.data) {
  ...
  static async cancelOrder(maker: string, order: string): Promise<CancelOrderResponse> {
    const response = await apiClient.post<{ success: boolean; data: CancelOrderResponse }>(
      '/api/limit-orders/cancel',
  ...
  static async cancelMultipleOrders(maker: string, orders: string[]): Promise<{ transactions: string[]; requestId: string }> {
    const response = await apiClient.post<{ success: boolean; data: { transactions: string[]; requestId: string } }>(
      '/api/limit-orders/cancel-multiple',
      { maker, orders, computeUnitPrice: 'auto' }
    );
    if (!response.success || !response.data) {
