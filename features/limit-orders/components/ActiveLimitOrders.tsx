...
export const ActiveLimitOrders: React.FC<ActiveLimitOrdersProps> = ({ tokenAddress, inputMint, outputMint }) => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            const response = await LimitOrderService.getOrders(walletAddress, "active", {
                inputMint,
                outputMint
            });
    ...
    const handleCancelOrder = async (orderAccount: string) => {
        ...
        try {
            ...
            const cancelResponse = await LimitOrderService.cancelOrder(walletAddress, orderAccount);
                        ...
                        <div className="flex items-start justify-between gap-4">
                            <div className="flex-1 space-y-1">
                                ...
                                <div className="text-xs text-gray-400">
                                    {order.expiredAt && <span>Expires: {new Date(order.expiredAt * 1000).toLocaleString()}</span>}
                                </div>
                                <div className="flex items-center gap-2">
                                    <a
                            ...
                            >
                                {cancellingOrder === order.account ? <Loader2 className="h-4 w-4 animate-spin" /> : <X className="h-4 w-4" />}
                            </Button>
                        </div>
                    </div>
                ))}
