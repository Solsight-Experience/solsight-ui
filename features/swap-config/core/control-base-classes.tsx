            ...
            <div className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                    <Button
                        ...
                        className={cn(
                            "bg-(--surface-btn) hover:bg-(--surface-btn-hover) ring-1 ring-transparent overflow-hidden whitespace-nowrap",
                            state.mode === "auto" && "ring-(--text-secondary)",
                            lock.forcedMode === "custom" && "opacity-50 cursor-not-allowed"
                        )}
                    >
                        Auto {auto !== null ? `(${this.format(auto)})` : ""}
                    </Button>
                    <div className={cn("relative grow ring-1 ring-transparent rounded-md", state.mode === "custom" && "ring-(--text-secondary)")}>
                        <NumbericInput
                            mode="string"
                            decimals={9}
                            ...
                            className="bg-(--surface-btn) border-(--border-default)"
                        />
                    </div>
                </div>
                {validation && (
                ...
                {lock.locked && lock.reason && <div className="text-xs text-(--text-muted)">{lock.reason}</div>}
            </div>
        );
    }
}
            ...
            <div className="space-y-2">
                <div className="relative">
                    <NumbericInput
                        ...
                        className="bg-(--surface-btn) border-(--border-default) pr-12"
                    />
                    <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-(--text-muted)">{this.suffix}</div>
                </div>
                {validation && (
                    <div className={cn("text-xs", validation.level === "warn" && "text-amber-400", validation.level === "error" && "text-rose-400")}>
                        {validation.message}
...
export abstract class SegmentedItem<TOption extends string> extends SwapConfigItem<{ value: TOption }> {
    ...
    renderExpandedForm({ state, onChange }: ExpandedFormProps<{ value: TOption }>): React.ReactNode {
        return (
            <div className="ml-auto flex w-fit max-w-full min-w-0 items-center gap-1 rounded-md bg-(--surface-btn) p-0.5">
                {this.options.map((option) => (
                    <Button
                        ...
                        className={cn(
                            "h-6 min-w-0 shrink px-2 text-xs font-normal gap-1 rounded text-(--text-secondary) hover:text-(--text-primary)",
                            state.value === option.id && "bg-(--surface-btn-active) text-(--text-primary) ring-1 ring-(--text-secondary)"
                        )}
                    >
        ...
        return (
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="w-full justify-between bg-(--surface-btn) border-(--border-default) hover:bg-(--surface-btn-hover)">
                ...
                <DropdownMenuContent className="w-[--radix-dropdown-menu-trigger-width] bg-popover border-(--border-default)">
                    {options.map((option) => (
                        <DropdownMenuItem key={option.id} onSelect={() => onChange({ value: option.id })} className="hover:bg-(--surface-btn-hover)">
                            <div className="flex items-center gap-2">
                                {option.node}
