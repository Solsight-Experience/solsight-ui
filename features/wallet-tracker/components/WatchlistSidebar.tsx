...
const WalletItem: React.FC<{ wallet: WatchedWallet; isSelected: boolean; onSelect: () => void }> = ({
  wallet,
  isSelected,
  onSelect,
}) => {
  ...
  const handleSaveLabel = (e: React.MouseEvent) => {
    ...
    updateWallet(
      ...
      {
        onSuccess: () => { toast.success('Label updated'); setEditing(false); },
      ...
      <div className="flex flex-col min-w-0 flex-1">
        {editing ? (
          <input
            ...
            className="bg-gray-700 border border-gray-500 rounded px-2 py-0.5 text-xs text-white
...
export const WatchlistSidebar: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full border-r border-gray-700/50 bg-gray-900/40">
      <div className="px-4 py-4 border-b border-gray-700/50">
        <h2 className="text-sm font-semibold text-white">Watchlist</h2>
        <p className="text-[11px] text-gray-500 mt-0.5">Track any Solana wallet</p>
        <div className="mt-3">
          <AddWalletForm />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 flex flex-col gap-1.5">
        ...
        {error && (
          <p className="text-xs text-red-400 text-center py-4">Failed to load watchlist</p>
        )}

        {!isLoading && !error && data?.wallets?.length === 0 && (
          <p className="text-xs text-gray-500 text-center py-4">No wallets tracked yet</p>
        )}
