...
import {
  ExternalLink, AlertTriangle, Coins, ArrowRightLeft, ArrowRight,
  ChevronLeft, ChevronRight, Bell, Copy, Check, ChevronDown,
  ArrowDownLeft, ArrowUpRight, Repeat2,
} from 'lucide-react';
import Link from 'next/link';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import {
  useWatchedPositions,
  useWatchedActivities,
} from '../hooks/useWatchedPortfolio';
...
const jupiterLogoUrl = (address: string) =>
  `https://img.jup.ag/tokens/${address}`;

const TokenLogo: React.FC<{ address?: string; uri?: string; symbol: string; size?: string }> = ({
  address, uri, symbol, size = 'size-5',
}) => {
  ...
  React.useEffect(() => { setIdx(0); }, [srcKey]);
...
const PositionsTab: React.FC<{ walletAddress: string }> = ({ walletAddress }) => {
  ...
  if (!data?.positions?.length) return (
    <div className="text-white/30 text-sm py-10 text-center">No positions found</div>
  );
        ...
        <tbody>
          {data.positions.map((position, idx) => (
            ...
            >
              <td className="px-4 py-3">
                ...
                >
                  ...
                  <div>
                    ...
                    {position.token.name && (
                      <div className="text-[10px] text-white/30 truncate max-w-[100px]">{position.token.name}</div>
                    )}
                  </div>
                </Link>
              </td>
              ...
              <td className="px-4 py-3 text-right text-[12px] font-semibold text-white">
...
const ActivityFilter: React.FC<{
  ...
  const label = selected.size === ALL_TYPES.length
    ? 'All types'
    : [...selected].map(t => FILTER_LABELS[t]).join(', ');
  ...
  return (
    <div ref={ref} className="relative self-start">
      ...
      {open && (
        <div className="absolute left-0 top-[calc(100%+6px)] z-20
          ...
          {(() => {
            ...
            return (
              <button
                onClick={() => {
                  if (!allChecked) ALL_TYPES.forEach(t => { if (!selected.has(t)) onToggle(t); });
                  else onToggleAll();
                }}
                className="flex items-center justify-between gap-3 px-3 py-2.5
                           ...
                           hover:bg-white/[0.05]"
              >
                <span className={`flex items-center gap-2 ${allChecked ? 'text-white' : 'text-white/50'}`}>
                  All types
                </span>
                <span className={`size-4 rounded flex items-center justify-center border transition-colors
...
const ActivityRow: React.FC<{ activity: any; isLast: boolean }> = ({ activity, isLast }) => {
  ...
  const transferToken =
    activity.type === 'TRANSFER_OUT' ? (tokenIn ?? activity.token) :
    activity.type === 'TRANSFER_IN'  ? (tokenOut ?? activity.token) :
    activity.token ?? null;
  const appName = activity.app?.name && activity.app.name !== 'Unknown' ? activity.app.name : null;

  return (
    <div className={`flex items-center gap-3 px-4 py-3.5 hover:bg-white/[0.025] transition-colors
      ...
      <div className="flex-1 min-w-0">
        {isSwap && tokenIn && tokenOut ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] font-mono text-red-400 tabular-nums leading-tight">
                  -{formatAmount(tokenIn.amount)}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <div className="text-[11px] font-mono text-emerald-400 tabular-nums leading-tight">
                  +{formatAmount(tokenOut.amount)}
                </div>
              </div>
            </div>
            ...
            {/* DEX name */}
            {appName && (
              <span className="shrink-0 px-2 py-0.5 rounded-md bg-violet-500/10 border border-violet-500/20
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <div className={`text-[11px] font-mono tabular-nums leading-tight
                ...
                {activity.type === 'TRANSFER_IN' ? '+' : '-'}{formatAmount(transferToken.amount)}
              </div>
            </div>
            {appName && (
              <span className="ml-1 px-2 py-0.5 rounded-md bg-white/[0.05] border border-white/[0.08]
          ...
          <div className="flex items-center gap-2">
            {appName && (
              <span className="text-[12px] text-white/50">{appName}</span>
            )}
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span
          className="text-[11px] text-white/30 tabular-nums whitespace-nowrap"
          title={new Date(activity.timestamp * 1000).toLocaleString()}
        >
...
const ActivityTab: React.FC<{ walletAddress: string }> = ({ walletAddress }) => {
  ...
  if (!data?.activities?.length) return (
    <div className="text-white/30 text-sm py-10 text-center">No activity found</div>
  );
  ...
  return (
    <div className="flex flex-col gap-3 mt-4">
      <ActivityFilter
        selected={selected}
        onToggle={toggleType}
        onToggleAll={() => { setSelected(new Set(['SWAP'])); setPage(1); }}
      />

      {!activities.length && (
        <div className="text-white/30 text-sm py-10 text-center">No activity for selected filters</div>
      )}
  ...
  return (
    <div className="p-5 flex flex-col gap-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-[18px] font-bold text-white font-mono tracking-tight">
