...
export const useCreateWalletAlert = (walletAddress: string) => {
    ...
    return useMutation({
        mutationFn: (dto: CreateWalletAlertDto) => walletAlertsService.create(walletAddress, dto),
...
export const useUpdateWalletAlert = (walletAddress: string) => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ alertId, dto }: { alertId: string; dto: UpdateWalletAlertDto }) => walletAlertsService.update(walletAddress, alertId, dto),
        onSuccess: () => qc.invalidateQueries({ queryKey: alertsKey(walletAddress) })
    });
};
...
export const useDeleteWalletAlert = (walletAddress: string) => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (alertId: string) => walletAlertsService.delete(walletAddress, alertId),
        onSuccess: () => qc.invalidateQueries({ queryKey: alertsKey(walletAddress) })
    });
};
