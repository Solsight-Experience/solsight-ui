...
export default function TokenDetailPage() {
    ...
    if (isLoading) {
        return (
            <div className="min-h-screen text-(--text-primary)">
                <div className="border-b border-(--border-subtle) p-4 animate-pulse">
                    <div className="max-w-7xl mx-auto flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-(--surface-btn)"></div>
                        <div className="flex flex-col gap-2">
                            <div className="h-6 w-32 bg-(--surface-btn) rounded"></div>
                            <div className="h-4 w-48 bg-(--surface-btn) rounded"></div>
                        </div>
                    </div>
                </div>
                <div className="max-w-7xl mx-auto p-4">
                    <div className="h-96 bg-(--surface-btn) rounded-lg"></div>
                </div>
            </div>
        );
    }
    ...
    if (error || !token) {
        return (
            <div className="min-h-[calc(100vh-65px)] bg-(--surface-page) text-(--text-primary) flex items-center justify-center px-4 relative overflow-hidden">
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-65px)] overflow-hidden">
            ...
            <div className="flex flex-row flex-1 overflow-hidden bg-(--surface-page) gap-2 p-2">
                {/* Left Column */}
                <div className="flex-1 overflow-hidden rounded-lg border border-(--border-faint)">
                    <ResizablePanelGroup orientation="vertical" className="flex-col">
                        <ResizablePanel defaultSize={60} minSize={20} className="flex flex-col bg-(--surface-page)">
                        ...
                        <ResizableHandle withHandle className="bg-(--surface-btn) hover:bg-violet-500 transition-colors" />

                        <ResizablePanel defaultSize={40} minSize={20} className="flex flex-col bg-(--surface-page) min-h-0">
                ...
                {/* Right Column */}
                <div className="w-85 shrink-0 overflow-y-auto scrollbar-thin rounded-lg">
